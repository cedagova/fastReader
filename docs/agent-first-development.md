...
- **Two agent guides, one substance**: `AGENTS.md` (Codex) and `CLAUDE.md`
  ...
  quirks. They are machine-local and untracked, so they can lag the code;
  this doc is the tracked, shareable statement of the approach, and
  [architecture.md](architecture.md) is the tracked map of the code. When a
  guide and a tracked doc disagree, the tracked doc and the build files win.
- **Current docs and historical records are kept apart.** Current docs are
  kept true to the code in the change that alters it; `docs/evidence/`,
  `docs/plans/`, `docs/product-definitions/` and `docs/audits/` are
  historical records, each marked so in its own `README.md` (the list is in
  [architecture.md](architecture.md#where-the-docs-are)).
...
| Shared build conventions (#205) | `build-logic/` is an included build of convention plugins: `conventions.android.application` and `conventions.android.library` apply SDK levels, the JVM target, lint gates, unit-test settings and the formatter to every Android module, `conventions.kotlin.library` applies the JVM target, formatter and public-surface settings to the Android-free `:reader-engine`, and `conventions.root` formats the root and build-logic scripts. A new module applies one of them and repeats none of those settings |
| Library API surface (#198) | `conventions.android.library` and `conventions.kotlin.library` turn on Kotlin explicit-API mode and the Kotlin Gradle plugin's ABI validation, so every reusable library's public surface is recorded in a committed `api/<module>.api` dump (`reader-auth/api/`, `reader-library/api/`, `reader-account/api/`, `reader-engine/api/`). `checkKotlinAbi` joins `./gradlew check` and the hosted gate and fails on any public change the dump does not show; after a deliberate change, `./gradlew updateKotlinAbi` rewrites the dump and the diff is the review. A dependency is `api` only when one of its types is in the surface on purpose, with the reason written in the module's build file; the rest stay `implementation`. Quirk: under AGP's built-in Kotlin the plugin finds no Android compilation, so `build-logic/src/main/kotlin/LibraryApiSurface.kt` feeds it the release classes |
| Library consumption (#207) | Another repository takes the libraries as a **source copy at a tagged library version** (owner decision D1 = A, 2026-09-25). `docs/library-consumption.md` holds the copy set — module directories, `build-logic/`, `.editorconfig`, the catalog keys — as the one list to extend; each module has `version` in its build file, a `CHANGELOG.md` and its own `consumer-rules.pro`. `scripts/library-copy-check.sh` (also in the hosted gate) copies the set into a throwaway standalone project under `$TMPDIR`, runs its minified build and the copied contract tests, and fails when a module's own keep rules no longer keep its `@Serializable` types; `--keep` leaves the project for a look |
...
| Hosted gate | `.github/workflows/checks.yml` on GitHub Actions (`ubuntu-latest`, free tier): `testDebugUnitTest`, `verifyRoborazziDebug`, `lint`, `spotlessCheck`, `checkKotlinAbi`, `:app:minifyReleaseWithR8` (no signing key), `scripts/library-copy-check.sh` and `compileDebugAndroidTestSources`, once per pull-request commit and once per `main` commit; `scripts/release.sh --publish` refuses a commit it has not passed on |
...
| Module map (#209) | Five modules — `:app` and the four reusable libraries `:reader-auth`, `:reader-library`, `:reader-account`, `:reader-engine` — with their dependency direction, what a new client copies versus uses as a pattern, and each library's host obligations: [architecture.md](architecture.md). Each library's own README is its detail; nothing below restates them |
| Reader auth library (#92, #93, #100) | Loop: `./gradlew :reader-auth:testDebugUnitTest` — Ktor mock engine, fake cipher, fake clock; no network or device. `:app`'s host obligations (nine-domain backup exclusion, no cleartext outside the debug-only `10.0.2.2` allowance in `app/src/debug/`, the `BuildConfig` → `ReaderAuthConfig` mapping, no committed value) are pinned by `ReaderAccountManifestTest` and `ReaderAccountConfigTest`; `scripts/release.sh` proves on the signed APK that the permissions are exactly `INTERNET` plus the platform self-permission and that the release manifest carries no `networkSecurityConfig` and no `usesCleartextTraffic`. See `reader-auth/README.md` and `reader-auth/CONTRACT.md` |
| Reader account-library module (#112, #147) | Loop: `./gradlew :reader-library:test` — no network, device or backend value, no golden. The reader-api operation tests drive a **real** `ReaderAuthClient` over a Ktor mock engine (`ReaderLibraryHarness` from the test fixtures), the transfer tests drive the session-less clients over a mock provider, and the sync tests drive the scripted `FakeReaderLibraryGateway`. Drift gate: `ReaderLibraryContractTest` against `reader-auth/contracts/` (update procedure in `reader-auth/contracts/PINNED.md`). See `reader-library/README.md` |
| Reader account pipeline (#200) | Loop: `./gradlew :reader-account:testDebugUnitTest` (controller, error mapping, graph hooks, copy store, shelf). The download and import pipelines are proved in `:app` against the real catalog (`AccountBookCopiesTest`, `AccountDownloadsTest`, `AccountImportsTest`). `:app` assembles it in `AppGraph` with one `ReaderAccountGraph(...)` call. See `reader-account/README.md` |
| Reader engines (#201) | Loop: `./gradlew :reader-engine:test` (the repository-wide `testDebugUnitTest` runs it too); plain JVM, no Robolectric. See `reader-engine/README.md` |
