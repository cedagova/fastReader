...
| Reader auth module and host (#92) | Two modules beside `:app` that share only the toolchain: `:reader-auth` (Android library, `com.cedagova.reader.auth`, declares `INTERNET`; see `reader-auth/README.md`) and `:reader-auth-host` (application id `com.cedagova.reader.auth.host`, never built by `scripts/release.sh`). Same loops: `./gradlew :reader-auth:testDebugUnitTest :reader-auth-host:testDebugUnitTest lint`; on a device `./gradlew :reader-auth-host:installDebug` → `adb shell am start -n com.cedagova.reader.auth.host/.MainActivity` → `adb exec-out screencap -p` shows the HTTPS probe line (`HTTPS 204 from www.gstatic.com`) → `adb logcat -d -s ReaderAuthHost:I AndroidRuntime:E`; `adb shell pm uninstall com.cedagova.reader.auth.host` removes it without touching FastReader's package. Only debug builds allow cleartext, to `10.0.2.2` alone (`reader-auth-host/src/debug/`); `aapt2 dump xmltree` of `:reader-auth-host:assembleRelease`'s unsigned APK is the check that a release carries no `networkSecurityConfig`. No Roborazzi goldens for the host |
