...
object AccountLibrarySchema {
     ...
     *
     * Later increments each take one forward step of their own: LEAF802 adds
     * import records, LEAF811 copy references, LEAF821 remote positions and
     * the resume record. Each adds a [MIGRATIONS] entry keyed by the version
     * it upgrades *from*, exactly as `CatalogSchema` does.
     */
    const val CURRENT_VERSION: Int = 1

    /**
     * Forward migrations keyed by the version they upgrade *from*. Empty while
     * version 1 is the only version there has ever been; the decoder's loop is
     * already written around it so the first extension is one entry and no new
     * machinery.
     */
    val MIGRATIONS: Map<Int, AccountLibraryMigration> = emptyMap()
}
