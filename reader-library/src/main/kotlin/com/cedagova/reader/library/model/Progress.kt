 ...
 * from the host's own position is the host's work, not the contract
 * module's.
 *
 ...
 *
 * This is the shape of the position a client *publishes*. This module publishes
 ...
 * value, a pipeline version and a structural fingerprint are the host's own
...
internal data class PutReaderProgressRequest(
    /** 0-100 inclusive: the whole percent the host shows the reader. */
    @SerialName("progress_percent") val progressPercent: Double,
    @SerialName("location") val location: ReaderPortableLocationV1,
    @SerialName("chapter_title") val chapterTitle: String? = null,
)
 ...
 * place — while the one this module *writes* is always built from
 * [ReaderEpubLocatorV1] (see `PortableProgress`), so
 ...
 *
 * This module states an account book here and nothing else: [publicationId] is
 ...
 * not modelled: this module never sends a digest of a book in a position.
 *
 ...
 *
 * [href] is the section — for an RSVP host, the spine path of the chapter the
 ...
 * send one; this module never writes it. An RSVP stream has no DOM to point a
 * CFI into, and a fabricated one would claim a precision an RSVP reader does not
 * have (A2).
 */
@Serializable
...
/** The locator format discriminator for an EPUB. The only one this module writes. */
