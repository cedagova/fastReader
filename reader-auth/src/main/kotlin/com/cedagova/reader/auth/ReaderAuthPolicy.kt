...
public object ReaderAuthPolicy {
     ...
     */
    public val MAX_INLINE_RETRY_AFTER: Duration = 30.seconds
     ...
     */
    internal val SESSION_CLEARING_REFRESH_CODES: Set<AuthErrorCode> = setOf(
    ...
    public const val EXPIRED_TOKEN_CODE: String = "auth.expired_token"

    /** The reader-api 502 code that is retried once after `Retry-After`. */
    public const val JWKS_DEPENDENCY_FAILED_CODE: String = "auth.jwks_dependency_failed"

    /** Whether a session whose access token expires at [expiresAt] must be refreshed at [now]. */
    internal fun needsRefresh(expiresAt: Instant, now: Instant): Boolean = expiresAt - now <= REFRESH_MARGIN
    ...
    internal fun retriesInline(retryAfter: Duration): Boolean = retryAfter <= MAX_INLINE_RETRY_AFTER
}
