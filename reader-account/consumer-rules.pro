# Keep rules every host of :reader-account needs when it shrinks with R8 (#200).
#
...
-keepclassmembers @kotlinx.serialization.Serializable class com.cedagova.reader.account.** {
    static ** Companion;
}

# `serializer()` on that companion.
-if @kotlinx.serialization.Serializable class com.cedagova.reader.account.** {
    static **$* *;
}
...
-if @kotlinx.serialization.Serializable class com.cedagova.reader.account.**
-keep,allowshrinking,allowoptimization,allowobfuscation,allowaccessmodification class com.cedagova.reader.account.<1>
